...
5. Closes the deposit invoice: `closeInvoice(depositInvoiceId)` — do **not** pass `{autoReturn: true}` here. Surplus per currency slot = `totalDeposited - requiredAmount`. Surplus is distributed proportionally per sender (by `effectiveSender`). Only the excess over the required amount is returned, not the full deposit. Surplus handling should be done after payouts complete. Passing `autoReturn: true` on close would trigger surplus returns concurrently with payout `payInvoice()` calls, creating a race for the escrow's token balance. **Note:** Close the deposit invoice first. If close fails (transient error), the timeout timer remains active as a safety net.
...
| `DEPOSIT_INVOICE_CREATED` | `EXPIRED` (with deposits, `first_deposit_at` non-null) | `dueDate` passed with deposits attributed but swap state not yet advanced — treat as equivalent to `PARTIAL_DEPOSIT`. Advance swap to `PARTIAL_DEPOSIT`, re-register timeout timer with remaining time, re-subscribe to events. |
| `DEPOSIT_INVOICE_CREATED` | `EXPIRED` (without deposits, `first_deposit_at` null) | `dueDate` passed and no deposits ever arrived. No timer was running and no event will advance this swap — it would be immortal. Transition to `FAILED`. |
...
| `CANCELLING` | `OPEN`, `PARTIAL`, or `EXPIRED` | `cancelInvoice()` not yet called or interrupted — call `cancelInvoice({ autoReturn: true })` to complete the cancellation |
