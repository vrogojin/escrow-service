...
**Note:** Steps 7, 8, and 9 are sub-steps within a single handler invocation (the `invoice:covered` event handler in SwapOrchestrator). They are numbered separately for clarity but execute as one sequential flow, not as separate asynchronous phases. However, this flow is **not transactional** — a crash between any sub-step leaves the swap in an intermediate state. The `CONCLUDING` transition (step 7) acts as a mutex/CAS guard and is persisted before any payout work; payout invoice IDs are then checkpointed incrementally as they are created. This ensures crash recovery can resume from the correct point (see architecture.md §Crash Recovery and §Event-Driven Flow `invoice:covered`).
...
4. Transitions: `PARTIAL_DEPOSIT → DEPOSIT_COVERED` (or `DEPOSIT_INVOICE_CREATED → DEPOSIT_COVERED`). This transition is **persisted to SwapStateStore** before proceeding. In normal operation, `DEPOSIT_COVERED` is short-lived — the handler immediately proceeds to closing and payout creation. However, a crash between this persist and the `CONCLUDING` persist (step 7) will leave the swap in `DEPOSIT_COVERED`, which crash recovery handles (see architecture.md §Crash Recovery).
5. Closes the deposit invoice: `closeInvoice(depositInvoiceId, { autoReturn: true })` — surplus is delegated to the SDK (see architecture.md §Event-Driven Flow). **Note:** Close the deposit invoice first. If close fails (transient error), the timeout timer remains active as a safety net.
6. Cancels the timeout timer (if running)
7. **Transitions to `CONCLUDING` FIRST** (persisted to SwapStateStore before any payout invoice is created) — this acts as the mutex/CAS guard preventing duplicate payout creation on re-entry. Payout invoice IDs are then checkpointed incrementally as each invoice is created: after payout A is created, `payout_a_invoice_id` is persisted (self-transition `CONCLUDING → CONCLUDING`); after payout B is created, `payout_b_invoice_id` is persisted (self-transition `CONCLUDING → CONCLUDING`). See §2.2 for payout invoice terms.
...
**Crash recovery warning:** When retrying `payInvoice()` during crash recovery, **omit the `amount` parameter**. The SDK will compute `remaining = requestedAmount - netCoveredAmount` and send only the uncovered remainder. If the payout was already completed before the crash, `remaining` will be `0` and the SDK throws `INVOICE_INVALID_AMOUNT` (safe to catch as success — this throw only occurs when `amount` is omitted; if `amount: '0'` is passed explicitly, the SDK follows a different code path and the behavior is unspecified; always omit `amount` when retrying). The SDK may also throw `INVOICE_TERMINATED` (invoice already closed/cancelled — treat as success) or `INVOICE_NOT_FOUND` (invoice token not loaded after restart — re-import via `importInvoice()`, then retry). Passing an explicit `amount` during retry bypasses the zero-remaining guard and causes a **double-payment**.
